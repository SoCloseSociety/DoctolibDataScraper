...
def extract_contact(soup: BeautifulSoup) -> list[str]:
    ...
    try:
        ...
        full_address = divs[0].text.strip() if divs else ""
        address = full_address.replace(practice_name, "").strip()
        contacts.append(f"{practice_name} -> {address}" if practice_name else address)
    except (IndexError, AttributeError) as exc:
        logger.debug("Address extraction issue: %s", exc)
    ...
    try:
        ...
        for section in sections:
            header = section.find("h4", class_="dl-profile-card-title")
            if header:
                contacts.append(header.text.strip())
                contacts.append("---")
            ...
            for entry in entries:
                ...
                label = label_el.text.strip() if label_el else ""
                if year or label:
                    contacts.append(f"{year} - {label}" if year else label)
    except AttributeError as exc:
        logger.debug("Degrees extraction issue: %s", exc)

    # --- contact info ---
    try:
        section = soup.find("div", class_="dl-profile-contact")
        if not section:
            return contacts
        divs = section.find_all("div")
        for div in divs:
            label_el = div.find("div", class_="dl-profile-label")
            value_el = div.find("div", class_="dl-profile-value")
            if label_el and value_el:
                contacts.append(f"{label_el.text.strip()} - {value_el.text.strip()}")
    except (IndexError, AttributeError) as exc:
        logger.debug("Contact info extraction issue: %s", exc)
...
if __name__ == "__main__":
    search_url = BASE_URL + "/docteurs"  # Example search URL
