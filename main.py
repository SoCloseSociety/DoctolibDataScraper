...
import requests
from nordpool.api import NordPoolAPI
...
if __name__ == "__main__":
    search_url = "https://www.doctolib.fr/docteur/paris"  # Example search URL
    profile_links = scrape_all_search_results(search_url)
    save_links_csv(profile_links, OUTPUT_LINKS_CSV)

    for link in profile_links:
        driver = create_driver()
        try:
            driver = safe_get(driver, BASE_URL + link, "profile")
            soup = BeautifulSoup(driver.page_source, "html.parser")
            address = extract_address(soup)
            skills = extract_skills(soup)
            degrees = extract_degrees(soup)
            contact = extract_contact(soup)

            # Save details to CSV
            df = pd.DataFrame({"address": [address], "skills": [skills], "degrees": [degrees], "contact": [contact]})
            df.to_csv(OUTPUT_DETAILS_CSV, index=False, mode="a", header=not os.path.exists(OUTPUT_DETAILS_CSV))
        except Exception as e:
            logger.error("Error processing profile %s: %s", link, e)
        finally:
            try:
                driver.quit()
            except Exception:
